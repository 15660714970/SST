...
class TrainDataset(Dataset):
    def __init__(self, data_root, crop_size, arg=True, bgr2rgb=True, stride=8):
        ...
        with open(f'{data_root}/split_txt/train_list1.txt', 'r') as fin:
        ...
        for i in range(len(hyper_list)):
            ...
            # hyper = sio.loadmat(hyper_path)
            # hyper = np.float32(np.array(hyper['cube']))
    ...
    def arguement(self, img, rotTimes, vFlip, hFlip, cutout_prob=0.5,  cutout_size=16):
        # Random rotation
    ...
    def __getitem__(self, idx):
        ...
        if self.arg:
            bgr = self.arguement(bgr, rotTimes, vFlip, hFlip, cutout_prob=0.5,  cutout_size=16)
            hyper = self.arguement(hyper, rotTimes, vFlip, hFlip, cutout_prob=0.5,  cutout_size=16)
        # if self.transform:
